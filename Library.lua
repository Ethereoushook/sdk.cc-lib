...
local LocalPlayer = Players.LocalPlayer;local InputService = game:GetService('UserInputService');
local TextService = game:GetService('TextService');
local CoreGui = game:GetService('CoreGui');
local Teams = game:GetService('Teams');
local Players = game:GetService('Players');
...
function Library:GetAccentColorSequence()
...
function Library:ApplyAccentGradient(Parent, Rotation, Mirrored, IsHud)
    ...
    Library:AddToRegistry(Gradient, {
        Color = function()
            return Mirrored and Library:GetAccentMirrorColorSequence() or Library:GetAccentColorSequence()
        end
    }, IsHud);
...
function Library:TrimString(Str)
    return (Str:gsub('^%s+', ''):gsub('%s+$', ''))
end;

function Library:SplitString(Str, Separator)
    local Result = {}
    local Start = 1

    while true do
        local FoundAt = string.find(Str, Separator, Start, true)
        if not FoundAt then
            table.insert(Result, string.sub(Str, Start))
            break
        end

        table.insert(Result, string.sub(Str, Start, FoundAt - 1))
        Start = FoundAt + #Separator
    end
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        local ContainerLabelGradient = Library:ApplyAccentGradient(ContainerLabel, 90, false, true);
        ContainerLabelGradient.Enabled = false;
        ...
        function KeyPicker:Update()
            ...
            ContainerLabelGradient.Enabled = State;
            ...
            Library.KeybindFrame.Size = UDim2.new(0, math.max(XSize + 16, 169), 0, YSize + 27)
        end;
...
do
    ...
    Library:ApplyAccentGradient(WatermarkStroke, 90, false, true);

    local WatermarkContainer = Library:Create('Frame', {
        BackgroundTransparency = 1;
        Position = UDim2.new(0, 5, 0, 0);
    ...
    Library:ApplyAccentGradient(KeybindStroke, 90, false, true);
...
function Library:SetWatermark(Text)
    ...
    for _, Part in next, RawParts do
        table.insert(Parts, Library:TrimString(Part))
    end
...
function Library:Notify(Text, Time)
    local XSize, YSize = Library:GetTextBounds(Text, Enum.Font.SourceSans, 14);

    YSize = math.max(YSize + 8, 25)
    ...
    Library:ApplyAccentGradient(NotifyStroke, 90, false, true);
    ...
    pcall(NotifyOuter.TweenSize, NotifyOuter, UDim2.new(0, XSize + 16, 0, YSize), 'Out', 'Quad', 0.4, true);
...
function Library:CreateWindow(...)
    ...
    Library:ApplyAccentGradient(OuterStroke, 90, false);
    ...
    function Window:AddTab(Name)
        ...
        local TabButtonWidth = Library:GetTextBounds(Name, Enum.Font.SourceSans, 13);
        ...
        Library:AddToRegistry(TabButtonLabel, {
            TextColor3 = function() return Color3.fromRGB(157, 157, 157) end;
        });
        ...
        local AccentLineGradient = Library:ApplyAccentGradient(AccentLine, 0, true);
        AccentLineGradient.Enabled = false;
        ...
        function Tab:ShowTab()
            ...
            AccentLineGradient.Enabled = true
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:ApplyAccentGradient(Highlight, 0, true);
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                function Tab:Resize()
                    ...
                    BoxOuter.Size = UDim2.new(1, 0, 0, 36 + Size + 4);
                end;
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        Library:ApplyAccentGradient(AccentLine, 0, true);
